...
def get_code_review(projectId, review_queries, llm):
    ...
    review_prompt = ChatPromptTemplate.from_template("""
        아래는 변경된 코드와 참고할 유사 코드입니다. 이를 바탕으로 리뷰를 작성해주세요.
        ...
        0. 참고 코드와 변경된 코드의 관계:
           - 유사한 점 및 차이점: [구체적으로 작성]
        1. 핵심 기능:
           - 변경된 코드의 주요 기능 설명
        2. 변경사항:
           - 추가된 기능 또는 변경된 로직 요약
        3. 주의 필요:
           - 잠재적인 문제점, 유지보수성, 성능 측면에서의 주의사항
        4. 수정해야 할 사항:
           - 수정이 필요하거나 개선 가능한 부분, 구체적인 코드 포함
        5. 개선 방향:
           - 참고 코드와 비교한 개선 가능성 (스타일, 성능, 유지보수성 등)
        ===
    
        응답은 반드시 위의 항목을 포함해야 하며, 필요한 경우 코드를 예로 들어 설명해주세요.
    """)
